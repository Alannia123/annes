...
class EducationExam(models.Model):
    ...
    valuation_line_ids = fields.One2many(
        'education.exam.valuation', 'exam_id', tracking=True,
    ...
    def action_confirm_exam(self):
        ...
        for subject in self.subject_line_ids:
            ...
            if curr_subject:
                divisions = self.env['education.class.division'].search([('class_id', '=', self.class_id.id)])
                for div in divisions:
                    valuation_line_id = self.env['education.exam.valuation'].create({
                                                        'exam_id': self.id,
                                                        'division_id': div.id,
                                                        'mark': self.mark,
